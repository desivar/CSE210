        ...
        using System;
using System.Collections.Generic; //we can use generic to handdle numbers

List<int> numbers = new List<int>(); //list that store the numbers

// Please note we could use a do-while loop here instead
int userNumber = -1;
while (userNumber != 0)
{
    Console.Write("Enter a number (0 to quit): ");// ask for a number and the program will stop when the usuary write zero
